...
<div class="layui-tab layui-tab-brief">
    <ul class="layui-tab-title">
        <li <?php if($tag == 10){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=10'])?>">全部</a></li>
        <li <?php if($tag == 1){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=1'])?>">未发货</a></li>
        <li <?php if($tag == 2){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=2'])?>">已发货</a></li>
        <li <?php if($tag == 3){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=3'])?>">无跟踪信息发</a></li>
        <li <?php if($tag == 4){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=4'])?>">退货</a></li>
        <li <?php if($tag == 5){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=5'])?>">退款</a></li>
        <li <?php if($tag == 6){?>class="layui-this" <?php }?>><a href="<?=Url::to(['order/ship-index?tag=6'])?>">换货</a></li>
    </ul>
</div>
...
<div class="lay-search" style="padding-left: 10px">
    ...
    <?php if(in_array($tag ,[1,2])){?>
        <div class="layui-inline">
            物流渠道
            <?= Html::dropDownList('OrderSearch[logistics_channels_id]', $searchModel['logistics_channels_id'], \common\models\Order::$logistics_channels_map,
        ...
        <table class="layui-table" style="text-align: center">
            ...
            <?php else: foreach ($list as $k => $v):
                ...
                <tr>
                    ...
                    <td align="left" rowspan="<?=$v['goods_count']?>">
                        物流渠道：<?= empty(\common\models\Order::$logistics_channels_map[$v['logistics_channels_id']])?'':\common\models\Order::$logistics_channels_map[$v['logistics_channels_id']] ?><br/>
                        物流单号：<?= $v['track_no'] ?>
                    </td>
                    <td align="left" rowspan="<?=$v['goods_count']?>">
                        下单时间：<?= date('Y-m-d H:i:s',$v['date'])?><br/>
                        预计到货时间：<?= empty($v['arrival_time'])?'':date('Y-m-d',$v['arrival_time']) ?><br/>
                        <?php if(in_array($tag,[2])){?>
