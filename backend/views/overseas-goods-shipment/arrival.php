...
<form class="layui-form layui-row" id="arrival" action="<?=Url::to(['overseas-goods-shipment/arrival'])?>">
    <div class="layui-col-xs12" style="padding: 0 10px;margin-top: 15px">
        <?php foreach ($list as $porder_id => $goods_v) {?>
        <div style="padding: 5px; background-color: #F2F2F2; margin-top: 10px;">
            <div class="layui-row layui-col-space15">
                <div class="layui-col-md12">
                    <div class="layui-card">
                        <div class="layui-card-header">
                            <span><b>订单号：</b><?=$porder_id;?></span>
                            <span style="padding-left: 10px">
                                <b>供应商单号：</b>「<?= $list[$porder_id][0]['supplier_name'] ?> 」<?=$list[$porder_id][0]['relation_no'];?>
                            </span>
                            <span style="float: right;padding-right: 10px">
                                <b>仓库：</b><?=$list[$porder_id][0]['warehouse_name']?>
                            </span>
                            <span style="float: right;padding-right: 10px;">
                                <b>采购时间：</b><?=date('Y-m-d H:i:s',$list[$porder_id][0]['purchase_time'])?>
                            </span>
                        </div>
                        <div class="layui-card-body">
                            <table class="layui-table" style="text-align: center;font-size: 13px">
                                ...
                                <tbody>
                                <?php foreach ($goods_v as $goods){
                                    $sku_no = $goods['sku_no']?>
                                    <tr class="goods_list" data-sku="<?=$sku_no?>">
                                        <td>
                                            <?php if(!empty($goods['goods_pic'])):?>
                                                <a href="<?=$goods['goods_pic']?>" data-lightbox="pic">
                                                    <img class="layui-upload-img" style="max-width: 200px;height: 100px"  src="<?=$goods['goods_pic']?>">
                                                </a>
                                            <?php endif;?>
                                        </td>
                                        ...
                                        <td><span style="color: green"> 0 / </span><?=$goods['num']?></td>
                                        <td>
                                            <input style="width: 70px;text-align:center;padding-left:0" data-num="<?=$goods['num']?>"  type="text" name="finish_num[<?=$goods['id']?>]" lay-verify="number" value="0" class="layui-input arrival_num">
                                        </td>
                                    </tr>
                                <?php }?>
                                </tbody>
...
<?=$this->registerJsFile("@adminPageJs/overseas-goods-shipment/arrival.js?v=".time()); ?>