    ...
    <div class="layui-card">
        <div class="layui-tab layui-tab-brief">
            <ul class="layui-tab-title">
                <li><a href="<?=Url::to(['goods-tiktok/platform-index'])?>">商品库</a></li>
                <li class="layui-this"><a href="<?=Url::to(['goods-tiktok/index'])?>">在线商品</a></li>
            </ul>
        </div>
<div class="lay-lists">
<!--<form class="layui-form">
    <blockquote class="layui-elem-quote quoteBox">
        <div class="layui-inline">
            <a class="layui-btn" data-type="url" data-title="添加商品" data-url="<?=Url::to(['goods-tiktok/create'])?>" data-callback_title = "商品列表" >添加商品</a>
        </div>
    </blockquote>
</form>-->
    <form>
<div class="layui-form lay-search" style="padding: 10px">
    ...
    <div class="layui-inline">
        店铺
        <?= \yii\helpers\Html::dropDownList('BaseGoodsSearch[shop_id]', null, \common\services\ShopService::getShopMap(\common\components\statics\Base::PLATFORM_TIKTOK),
    ...
    <button class="layui-btn" data-type="search_lists">搜索</button>
        <button class="layui-btn layui-btn-normal" data-type="export_load_lists" data-method="post" data-url="<?=Url::to(['goods-tiktok/export'])?>">导出</button>
    </div>
</div>
    </form>
...
<div class="layui-form" style="padding: 10px">
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-normal batch_o_category_btn" data-url="<?=Url::to(['goods-tiktok/batch-category'])?>">批量设置Tiktok类目</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-danger batch_del_btn" data-url="<?=Url::to(['goods-tiktok/batch-del'])?>" >批量删除</a>
    </div>
</div>
    <div class="layui-card-body">
<table id="goods-tiktok" class="layui-table" lay-data="{url:'<?=Url::to(['goods-tiktok/list'])?>', height : 'full-190', cellMinWidth : 95, page:{limits:[20, 50, 100, 500, 1000]},method :'post', limit : 20}" lay-filter="goods-tiktok">
    <thead>
    <tr>
        ...
        <th lay-data="{field: 'o_category_name', align:'left',width:130}">Tiktok类目</th>
...
<script type="text/html" id="goodsListBar">
    <a class="layui-btn layui-btn-xs" lay-event="open" data-width="950px" data-height="550px" data-url="<?=Url::to(['goods-tiktok/update-price'])?>?id={{ d.id }}" data-title="编辑价格" data-callback_title="商品列表">价格</a>
    {{#  if(d.status != <?=\common\services\goods\GoodsService::PLATFORM_GOODS_STATUS_UNCONFIRMED?>){ }}
    <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="update" data-url="<?=Url::to(['goods-tiktok/update'])?>?id={{ d.id }}" data-title="编辑商品" data-callback_title="商品列表">编辑</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete" data-url="<?=Url::to(['goods-tiktok/delete'])?>?id={{ d.id }}">删除</a>
    {{# } }}
</script>
