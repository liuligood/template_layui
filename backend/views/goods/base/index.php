...
<div class="layui-fluid">
    <div class="layui-card">
        <?php if(in_array($platform_type ,[Base::PLATFORM_HEPSIGLOBAL,Base::PLATFORM_MICROSOFT,Base::PLATFORM_RDC,Base::PLATFORM_WORTEN,Base::PLATFORM_NOCNOC])){ ?>
        <div class="layui-tab layui-tab-brief">
            <ul class="layui-tab-title">
                <?php if (!in_array($platform_type,[Base::PLATFORM_NOCNOC])) {?>
                    <li><a href="<?=Url::to(['goods-'.$url_platform_name.'/platform-index'])?>">商品库</a></li>
                <?php }?>
                    <li class="layui-this"><a href="<?=Url::to(['goods-'.$url_platform_name.'/index'])?>">在线商品</a></li>
                <?php if (in_array($platform_type,[Base::PLATFORM_HEPSIGLOBAL,Base::PLATFORM_OZON])) {?>
                ...
                <?php }?>
                <?php if (in_array($platform_type,[Base::PLATFORM_RDC,Base::PLATFORM_WORTEN,Base::PLATFORM_NOCNOC])) {?>
...
<div class="layui-form lay-search" style="padding: 10px">
    ...
    <button class="layui-btn" data-type="search_lists">搜索</button>
        <button class="layui-btn layui-btn-normal" data-type="export_load_lists"  data-method="post" data-url="<?=Url::to(['goods-'.$url_platform_name.'/export'])?>">导出</button>
        <?php if ($platform_type == Base::PLATFORM_HEPSIGLOBAL) {?>
        <button class="layui-btn layui-btn-primary ys-upload-weight" lay-data="{accept: 'file'}">导入重量</button>
        <?php }?>
    </div>
</div>
    </form>
...
<div class="layui-form" style="padding-left: 10px;margin-top: 10px">
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-normal batch_o_category_btn" data-url="<?=Url::to(['goods-'.$url_platform_name.'/batch-category'])?>">批量设置<?=$platform_name?>类目</a>
    </div>
    <?php if ($platform_type == Base::PLATFORM_HEPSIGLOBAL) {?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm js-batch" data-title="批量收藏" data-url="<?=Url::to(['goods-'.$url_platform_name.'/add-ads'])?>">批量收藏</a>
    </div>
    <?php }?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-danger batch_del_btn" data-url="<?=Url::to(['goods-'.$url_platform_name.'/batch-del'])?>" >批量删除</a>
    </div>
</div>
    <div class="layui-card-body">
<table id="goods-<?=$url_platform_name?>" class="layui-table" lay-data="{url:'<?=Url::to(['goods-'.$url_platform_name.'/list'])?>', height : 'full-190', cellMinWidth : 95, page:{limits:[20, 50, 100, 500, 1000]}, method :'post',limit : 20}" lay-filter="goods-<?=$url_platform_name?>">
    <thead>
    <tr>
        <th lay-data="{type: 'checkbox', fixed:'left', width:50}">ID</th>
        <th lay-data="{ width:180, align:'left',templet:'#goodsNoTpl'}">商品编号</th>
        <th lay-data="{field: 'shop_name', align:'left',width:130}">店铺名称</th>
        <th lay-data="{field: 'category_name', align:'left',width:130}">平台类目</th>
        <th lay-data="{field: 'o_category_name', align:'left',width:130}"><?=$platform_name?>类目</th>
...
<script type="text/html" id="goodsListBar">
    <a class="layui-btn layui-btn-xs" lay-event="open" data-width="950px" data-height="550px" data-url="<?=Url::to(['goods-'.$url_platform_name.'/update-price'])?>?id={{ d.id }}" data-title="编辑价格" data-callback_title="商品列表">价格</a>
    ...
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete" data-url="<?=Url::to(['goods-'.$url_platform_name.'/delete'])?>?id={{ d.id }}">删除</a>
    {{# } }}
</script>
...
<script type="text/html" id="goodsNoTpl">
    {{d.goods_no}}
    {{# if(d.follow_claim == 1){ }}
    <span class="layui-badge layui-bg-green" >跟</span>
    {{# } }}
    {{# if(d.ad_status == 2){ }}
    <span class="layui-badge layui-bg-orange">藏</span>
    {{# } }}
</script>
