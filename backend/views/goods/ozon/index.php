...
<div class="layui-fluid">
    <div class="layui-card">
        <div class="layui-tab layui-tab-brief">
            <ul class="layui-tab-title">
                ...
                <li class="layui-this"><a href="<?=Url::to(['goods-ozon/index'])?>">在线商品</a></li>
    ...
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-normal js-batch" data-title="发布" data-url="<?=Url::to(['goods-ozon/batch-release'])?>">批量发布</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm js-batch" data-title="批量收藏" data-url="<?=Url::to(['goods-ozon/add-ads'])?>">批量收藏</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-danger batch_del_btn" data-url="<?=Url::to(['goods-ozon/batch-del'])?>" >批量删除</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm  batch_invalid_btns"  data-url="<?=Url::to(['goods-ozon/set-warehouse'])?>">批量设置仓库</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm js-batch" data-title="批量恢复商品" data-url="<?=Url::to(['goods-ozon/batch-resume'])?>">批量恢复商品</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-normal batch_init_info_btn" data-url="<?=Url::to(['goods-ozon/init-info'])?>" >重置商品</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-normal valid_btn" data-url="<?=Url::to(['goods-ozon/batch-claim?type=1'])?>" >选中批量认领</a>
    </div>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm layui-btn-normal batch_update_price_btn" data-url="<?=Url::to(['goods-ozon/batch-update-price?'])?>" >批量锁定价格</a>
    </div>
</div>
    <div class="layui-card-body">
<table id="goods-ozon" class="layui-table" lay-data="{url:'<?=Url::to(['goods-ozon/list'])?>', height : 'full-190', cellMinWidth : 95, page:{limits:[20, 50, 100, 500, 1000]}, method :'post',limit : 20}" lay-filter="goods-ozon">
    <thead>
    <tr>
...
<script type="text/html" id="goodsImgTpl">
    <a href="{{d.image}}" data-lightbox="pic">
        <img class="layui-circle" src="{{d.short_image}}" width="100"/>
    </a>
</script>
...
<script type="text/html" id="goodsTpl">
    ...
    参考范围：[ {{d.price_range.start||''}} - {{d.price_range.end||''}} ] 成本价：{{d.min_cost||''}}
...
<script type="text/html" id="shopTpl">
    ...
    Ozon类目：<a style="color: #00a0e9" lay-event="update" data-url="<?=Url::to(['category/mapping-category'])?>?category_id={{ d.category_id }}&platform_type=30"  data-title="Ozon映射">{{d.o_category_name || '无'}}</a><br/>
    ...
    {{#  if(d.ad_status == 2){ }}
    <span  style="padding: 0px 10px;float: left" class="layui-btn layui-btn-normal layui-btn-xs" >藏</span>
    {{# } }}
...
<script type="text/html" id="goodsListBar">
    ...
    {{# } }}
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete" data-url="<?=Url::to(['goods-ozon/delete'])?>?id={{ d.id }}">删除</a>
    {{#  if(d.gs_status == <?=\common\models\GoodsShop::STATUS_SUCCESS?> && d.ad_status == 1){ }}
    <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="operating" data-title="加入收藏" data-url="<?=Url::to(['goods-ozon/add-ad'])?>?id={{ d.id}}">加入收藏</a>
    {{# } }}
    {{# }else{ }}
    <!--<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="operating" data-title="恢复商品" data-url="<?=Url::to(['goods-ozon/resume'])?>?id={{ d.id }}">恢复商品</a>-->
    {{# } }}
</script>
