    ...
    <div class="layui-card">
        <div class="layui-tab layui-tab-brief">
            <ul class="layui-tab-title">
                <li><a href="<?=Url::to(['goods-wildberries/platform-index'])?>">商品库</a></li>
                <li class="layui-this"><a href="<?=Url::to(['goods-wildberries/index'])?>">在线商品</a></li>
                <li><a href="<?=Url::to(['goods-wildberries/overseas-index'])?>">海外仓</a></li>
            </ul>
        </div>
        <div class="lay-lists">
            <form>
                <div class="layui-form lay-search" style="padding: 10px">
                    ...
                    <div class="layui-inline">
                        店铺
                        <?= \yii\helpers\Html::dropDownList('BaseGoodsSearch[shop_id]', null, \common\services\ShopService::getShopMap(\common\components\statics\Base::PLATFORM_WILDBERRIES),
                    ...
                    <div class="layui-inline">
                        <div style="padding-left: 10px">
                            <input class="layui-input search-con" type="checkbox" value="1" name="BaseGoodsSearch[un_o_category_name]" lay-skin="primary" title="未设置Wildberries类目">
                        </div>
                    </div>
                    <div class="layui-inline layui-vertical-20">
                        <button class="layui-btn" data-type="search_lists">搜索</button>
                        <button class="layui-btn layui-btn-normal" data-type="export_load_lists" data-method="post" data-url="<?=Url::to(['goods-wildberries/export'])?>">导出</button>
                    </div>
                </div>
            </form>
            ...
            <div class="layui-form" style="padding: 10px">
                <div class="layui-inline">
                    <a class="layui-btn layui-btn-sm layui-btn-normal batch_o_category_btn" data-url="<?=Url::to(['goods-wildberries/batch-category'])?>">批量设置Wildberries类目</a>
                </div>
                <div class="layui-inline">
                    <a class="layui-btn layui-btn-sm layui-btn-danger batch_del_btn" data-url="<?=Url::to(['goods-wildberries/batch-del'])?>" >批量删除</a>
                </div>
            </div>
            <div class="layui-card-body">
                <table id="goods-wildberries" class="layui-table" lay-data="{url:'<?=Url::to(['goods-wildberries/list'])?>', height : 'full-190', cellMinWidth : 95, page:{limits:[20, 50, 100, 500, 1000]}, method :'post',limit : 20}" lay-filter="goods-wildberries">
                    <thead>
                    <tr>
                        ...
                        <th lay-data="{field: 'o_category_name', align:'left',width:130}">Wildberries类目</th>
...
<script type="text/html" id="goodsListBar">
    <a class="layui-btn layui-btn-xs" lay-event="open" data-width="950px" data-height="550px" data-url="<?=Url::to(['goods-wildberries/update-price'])?>?id={{ d.id }}" data-title="编辑价格" data-callback_title="商品列表">价格</a>
    {{#  if(d.status != <?=\common\services\goods\GoodsService::PLATFORM_GOODS_STATUS_UNCONFIRMED?>){ }}
    <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="update" data-url="<?=Url::to(['goods-wildberries/update'])?>?id={{ d.id }}" data-title="编辑商品" data-callback_title="商品列表">编辑</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete" data-url="<?=Url::to(['goods-wildberries/delete'])?>?id={{ d.id }}">删除</a>
    {{# } }}
</script>
...
<script>
    const tableName="goods-wildberries";
