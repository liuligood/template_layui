...
<div class="layui-tab layui-tab-brief">
    <ul class="layui-tab-title">
        <li <?php if($tag == OverseasGoods::OVERSEAS_GOODS_STATUS_UNTREATED){?>class="layui-this" <?php }?>><a href="<?=Url::to(['overseas-goods/index?tag='.OverseasGoods::OVERSEAS_GOODS_STATUS_UNTREATED])?>">未完善</a></li>
        <li <?php if($tag == OverseasGoods::OVERSEAS_GOODS_STATUS_NORMAL){?>class="layui-this" <?php }?>><a href="<?=Url::to(['overseas-goods/index?tag='.OverseasGoods::OVERSEAS_GOODS_STATUS_NORMAL])?>">正常</a></li>
    </ul>
</div>
<div class="lay-lists">
<form>
<div class="layui-form lay-search" style="padding: 10px">
    <div class="layui-inline">
        商品编号
        <textarea name="OverseasGoodsSearch[goods_no]" class="layui-textarea search-con" style="height: 39px; min-height:39px"></textarea>
    </div>
    <div class="layui-inline">
        SKU
        <textarea name="OverseasGoodsSearch[sku_no]" class="layui-textarea search-con" style="height: 39px; min-height:39px"></textarea>
    </div>
    <div class="layui-inline">
        标题
        <input class="layui-input search-con" name="OverseasGoodsSearch[goods_name]" autocomplete="off">
    </div>
    <div class="layui-inline">
        平台类目
        <?php if($tag != 3){?>
        <div id="div_category_id" style="width: 180px;"></div>
        <input id="category_id" class="layui-input search-con" type="hidden" name="OverseasGoodsSearch[category_id]" autocomplete="off">
        <?php }else{?>
        <?= Html::dropDownList('OverseasGoodsSearch[category_id]', null, $category_arr,
            ['lay-ignore'=>'lay-ignore','prompt' => '全部','class'=>'layui-input search-con ys-select2' ,'lay-search'=>'lay-search' ]) ?>
        <?php }?>
    </div>
    <div class="layui-inline" style="width: 200px;">
        店铺(已认领)
        <select name="OverseasGoodsSearch[claim_shop_name]" class="layui-input search-con ys-select2" data-placeholder="请选择" lay-ignore >
    ...
    <div class="layui-inline" style="width: 220px">
        <label>排除已认领店铺</label>
        <select name="OverseasGoodsSearch[un_claim_shop_name][]" class="layui-input search-con ys-select2" multiple="multiple" data-placeholder="全部" lay-ignore >
            <?php foreach ($shop_arr as $ptype_v){ ?>
    ...
    <div class="layui-inline">
        <label>创建时间</label>
        <input  class="layui-input search-con ys-datetime" name="OverseasGoodsSearch[start_add_time]" id="start_add_time" autocomplete="off">
    </div>
    <span class="layui-inline layui-vertical-20">
        -
    </span>
    <div class="layui-inline layui-vertical-20">
        <input  class="layui-input search-con ys-datetime" name="OverseasGoodsSearch[end_add_time]" id="end_add_time" autocomplete="off">
    </div>
    <div class="layui-inline">
        状态
        <?= Html::dropDownList('OverseasGoodsSearch[status]', null, [
...
<div class="layui-form" style="padding: 10px">
    ...
    <?php if($tag == OverseasGoods::OVERSEAS_GOODS_STATUS_NORMAL){ ?>
    <div class="layui-inline">
        <a class="layui-btn layui-btn-sm js-batch" data-title="移入精选" data-url="<?=Url::to(['goods/batch-add-finds?source_method='.$source_method])?>" >批量精选</a>
    </div>
    <?php } ?>
    <!--<div class="layui-inline">
...
<table id="goods" class="layui-table" lay-data="{url:'<?=Url::to(['overseas-goods/list?tag='.$tag.'&source_method='.$source_method])?>', height : 'full-210',method :'post', cellMinWidth : 95, page:{limits:[20, 50, 100, 500, 1000]}, limit : 20}" lay-filter="goods">
    <thead>
    <tr>
...
<script type="text/html" id="goodsListBar">
    ...
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="update" data-url="<?=Url::to(['goods/update'])?>?id={{ d.id }}&ogid={{ d.ogid }}" data-title="编辑商品" data-callback_title="商品列表">编辑</a>
    </div>
    <div class="layui-inline">
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete" data-url="<?=Url::to(['overseas-goods/delete'])?>?id={{ d.ogid }}">移除</a>
    </div>
