...
class SiteController extends BaseController
{
    ...
    public function actionPassword()
    {
    	...
    	$req = Yii::$app->request;
    	$use = AdminUser::findOne($id);
    	$opass = $req->post('oldpassword');
    	if($req->isPost){
    		Yii::$app->response->format=Response::FORMAT_JSON;
    		if (!$use->validatePassword($opass)){
    			return $this->FormatArray(self::REQUEST_FAIL,'旧密码错误');
    		}else {
    			$pass =$req->post('password');
    			$use->password_hash= Yii::$app->getSecurity()->generatePasswordHash($pass);
    		if ($req->isPost){
    			Yii::$app->response->format=Response::FORMAT_JSON;
    			if ($use->load($req->post(),'') && $use->save()) {
    ...
    public function actionInfo(){
    	$id = Yii::$app->user->identity->id;
    	$adminModel = AdminUser::findOne($id);
    	$all_role = Assignment::getAllItems();

    	return $this->render('info',['admin_user'=>$adminModel->toArray(),]);
    }
