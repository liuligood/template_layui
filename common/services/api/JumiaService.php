 ...
 */
class JumiaService extends BaseSellerCenterApiService
{
    ...
    public function getBaseUri()
    {
        ...
        switch ($shop['country_site']) {
            case 'NG':
                return 'https://sellercenter-api.jumia.com.ng';
                break;
            case 'KE':
                return 'https://sellercenter-api.jumia.co.ke';
                break;
            case 'EG':
                return 'https://sellercenter.jumia.com.eg';
                break;
            case 'MA':
                return 'https://sellercenter.jumia.ma';
                break;
            case 'GH':
                return 'https://sellercenter.jumia.com.gh';
                break;
            case 'UG':
                return 'https://sellercenter.jumia.ug';
                break;
        }
        return '';
    }
    ...
    public function dealGoodsInfo($goods, $base_goods, $goods_shop)
    {
        ...
        //限制150
        $colour =empty($goods['ccolour'])? $goods['colour'] :'';
        ...
        if (empty($goods_name)) {
            ...
            $goods_name = (new JumiaPlatform())->filterContent(CommonUtil::usubstr($goods_name, $len));
        }
        ...
        $params = [];
        $category_id = trim($base_goods['o_category_name']);
        ...
        $jumia_platform = (new JumiaPlatform())->setCountryCode($country);
        foreach ($category_attr as $attr_v) {
            ...
            if ($attr_v['FeedName'] == 'ShortDescription') {
                $params[$attr_v['FeedName']] = $jumia_platform->descDeal($base_goods);
                continue;
            }
        ...
        $info['Description'] = $jumia_platform->dealContent($base_goods);
        $info['Brand'] = 'Generic';
    ...
    public function getShipmentProvidersName()
    {
        $shop = $this->shop;
        switch ($shop['country_site']) {
            case 'NG':
                return 'NG-JG-Seko-Shenzhen-Seko-Station';
                break;
            case 'KE':
                return 'KE-JG-Seko-Shenzhen-Seko-Station';
                break;
            case 'EG':
                return '';
                break;
    ...
    public function setTrackingNumber($order_id, $tracking_number, $carrier_code = null, $tracking_url = null)
    {
        ...
        $ids = [];
        $track_no = '';
        $logistics_channels_name = $this->getShipmentProvidersName();
        foreach ($items as $v) {
            $ids[] = $v['OrderItemId'];
            $track_no = $v['TrackingCode'];
            //$logistics_channels_name = $v['ShippingProviderType'];
        }
        $result = $this->request('GET', 'SetStatusToReadyToShip', ['OrderItemIds' => json_encode($ids), 'DeliveryType' => 'dropship','ShippingProvider'=>$logistics_channels_name]);

        if (empty($result['SuccessResponse'])) {
            CommonUtil::logs('jimia set-tracking-number error order_id:' . $order_id . ' result:' . json_encode($result), 'fapi');
        }
        ...
        if (empty($order['track_no'])) {
            if (empty($track_no)) {
                ...
                foreach ($items as $v) {
                    $track_no = $v['TrackingCode'];
                    $logistics_channels_name = $v['ShipmentProvider'];
                }
            }
